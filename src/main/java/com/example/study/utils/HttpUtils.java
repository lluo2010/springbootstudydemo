...
public class HttpUtils {
    ...
    public static String post(String url, List<Header> headers, Map params) throws IOException{
        List<NameValuePair> list=new ArrayList<>();
        for(Object key: params.keySet())
            list.add(new BasicNameValuePair(String.valueOf(key), String.valueOf(params.get(key))));
    ...
    public static String postHtml(String uri,String content) throws  IOException{
        List<Header> headers=Arrays.asList(new BasicHeader(HttpHeaders.CONTENT_ENCODING, "utf-8"),new BasicHeader(HttpHeaders.CONTENT_TYPE, ContentType.TEXT_HTML.getMimeType()));
        return request(new HttpPost(uri),headers,null,content);
    }
    ...
    public static File getFile(String url, List<Header> headers, Map<String, Object> params, String pathDir, String fileName) throws Exception {
        StringBuilder sb=new StringBuilder(url);
        boolean flag=url.indexOf("?")==-1;
        for(String key:params.keySet()){
            (flag?sb.append("?"):sb.append("&"))
        ...
        return requestFile(new HttpGet(sb.toString()), headers, null, null,
                pathDir, fileName);
    }

    private static File requestFile(HttpRequestBase httpRequest, List<Header> headers, List<NameValuePair> nvps, String postData,
                                    String pathDir, String fileName) throws  Exception,
            ClientProtocolException {
        Date start=new Date();
        File file = null;
        CloseableHttpResponse response = null;
